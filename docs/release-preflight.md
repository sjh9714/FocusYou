...
`scripts/release_preflight.sh`는 릴리스 직전 브랜치/태그/체인지로그 불일치를 자동으로 차단합니다.

## 왜 필요한가

- `main`/`develop` 분기 상태에서 잘못된 커밋으로 태그를 찍는 실수 방지
- `CHANGELOG.md` 최상단 버전과 태그 버전 불일치 방지
- 태그가 실제 릴리스 커밋(`main == develop`)을 가리키는지 검증
...
- `--skip-fetch`
  - `git fetch origin main develop --tags` 생략
  - 오프라인/특수 상황에서만 사용 권장
