...
> **2025 시장 업데이트:** Focused Work(4.9★, Flowmodoro+자동차단), Flow(100K+ daily users, Pro 차단)
> 등이 "타이머+차단 연동"을 제공하기 시작. "연동 자체"만으로는 차별화 부족.
> Focus You의 진짜 차별점은 **"연동의 신뢰성"과 "한국형 프리셋"**이다.

### 차별화 포지션

1. **신뢰/원복 확실성:** 3중 안전장치(AppDelegate 종료 정리 + stale 마커 복구 + LaunchAgent). 세션 완료/취소/크래시 어떤 상황에서도 차단 100% 원복.
2. **한국형 기본 프리셋:** 카카오톡, 네이버, 디시인사이드, 쿠팡플레이 등 한국 서비스 완벽 지원. 글로벌 앱들이 약한 구간.
3. **초저마찰:** 메뉴바 1초 시작, 유휴 CPU 0%. 집중 앱이 집중을 방해하지 않음.
...
```
1. 메뉴바 원클릭
   KR: "메뉴바에서 1초 시작" / "시작하면 차단, 끝나면 자동 해제"
   EN: "Start in one click" / "Block on start. Unblock on finish."
   화면: 팝오버 타이머 + "차단: n개 사이트, n개 앱" 요약 필수 노출

2. 3가지 모드
   KR: "당신의 리듬대로" / "뽀모도로 · 자유 · Flowmodoro"
   EN: "Focus your way" / "Pomodoro, Free, Flowmodoro"
   화면: 모드 토글 + 파이차트 + FOCUS/BREAK 배지

3. 차단 시스템
   KR: "프리셋 1탭이면 끝" / "SNS · 뉴스 · 동영상 · 게임, 사이트+앱 동시 차단"
   EN: "One tap to block" / "Sites + apps blocked together"
   화면: 카테고리 그리드 + 선택된 항목 리스트

4. 통계 + 스트릭
   KR: "오늘과 이번 주 한눈에" / "집중 시간 · 모드별 기록 · 스트릭"
   EN: "See your progress" / "Time, modes, streaks"
...
### 3초 설명
```
KR: "시작 버튼 하나로 — 타이머와 차단이 동시에. 끝나면 자동 해제.
     3중 안전장치로 원복 100% 보장."

EN: "One button starts it all — timer + blocking together.
     Triple safety net guarantees full restore."
```
